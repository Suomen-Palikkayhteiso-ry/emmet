"""Set email verified command - set user's email as verified."""
...
def set_email_verified(
    ...
    click.echo(f"Setting email as verified for {email}...")
    ...
    try:
        ...
        click.echo(f"✓ Email set as verified for {username} ({email})")

    except KeycloakError as e:
        logger.error(f"Error setting email as verified for {email}: {e}")
        return
